    ...
    def make_listwise_text(self, question, ctxs, sep='|'):
        ...
        for i in range(len(ctxs)):
            if self.args.n_special_tokens >= 1:
                if self.args.view_change == 'original':
                    view_list = [x for x in range(0, self.args.n_special_tokens)]
                elif self.args.view_change == 'reverse_views':
                    view_list = [x for x in range(0, self.args.n_special_tokens)][::-1]
                elif self.args.view_change == 'random_shuffle_views':
                    view_list = [x for x in range(0, self.args.n_special_tokens)]
                    random.shuffle(view_list)
                elif self.args.view_change == 'one_number_views':
                    view_list = [self.args.view_number for x in range(0, self.args.n_special_tokens)]

                special_str = "".join([f"<extra_id_{x}>" for x in view_list])
...
def main():
    ...
    # view settings for the special tokens
    parser.add_argument('--view_change', default='original', type=str)

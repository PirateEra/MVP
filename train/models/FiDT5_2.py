...
class FiDT5(transformers.T5ForConditionalGeneration):
    ...
    def listwise_ranknet_loss(self, logits, label_dist, use_weight=True):
        B, N = logits.shape
    ...
    def forward(
        ...
        **kwargs
        ...
        if labels is None:
            ...
            # list_view_token = encoder_outputs[0]
        ...
        if labels is not None:
            ...
            if label_dist is not None:
                ...
                if self.local_weight > 0: 
                    ...
                    # IPython.embed()
                    # exit()
                    _t = passage_embed.view(bsz, self.n_special_tokens, self.n_passages, -1)
                    _t = _t.permute(0, 2, 1, 3)
                    _t = _t.reshape(bsz * self.n_passages * self.n_special_tokens, -1)
                    _bsz = bsz * self.n_passages

                    orthoognal_loss = self.orthogonal_loss(_t, _bsz)
            ...
            if orthogonal_loss > 0:
                ...
                loss = global_loss
            # loss = list_loss + point_loss + final_loss

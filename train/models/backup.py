...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = encoder_outputs[0]
        # soyoung: reshape attention_mask
        n_psgs = self.encoder.n_passages
        encoder_output_k = self.encoder.encoder_output_k
        bsize = attention_mask.shape[0]
        attention_mask = attention_mask.view([bsize, n_psgs, -1])
        attention_mask = attention_mask[:, :, :encoder_output_k].reshape(bsize, -1)
        if self.model_parallel:
            torch.cuda.set_device(self.decoder.first_device)

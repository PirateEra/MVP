        ...
        for ns in "${n_special_tokens[@]}"; do
          for lw in "${local_weights[@]}"; do
            for warmup in "${warmup_ratio[@]}"; do
              ...
              CUDA_VISIBLE_DEVICES=0 python3 train.py --name $model_name \
                --seed ${s} \
                --base_model t5-base \
                --train-files ./data/train.jsonl \
                --eval-files ./data/validation.jsonl \
                --do_train --learning_rate $lr --gradient_accumulation_steps 2 --train_batch_size 16 --eval_batch_size 1 --num_workers 0 \
                ...
                --warmup_steps $warmup \
                --output_dir ../checkpoints_max_aggregation/ \
...
#                 --train-files ./data/train.jsonl \
#                 --eval-files ./data/validation.jsonl \
#                 --do_train --learning_rate $lr --gradient_accumulation_steps 16\
